...
class ZoneDetector:
    ...
    def validate_leg_out_breakout_distance(self, data: pd.DataFrame, start_idx: int, 
        ...
        """
        ENHANCED: Leg-out must break zone boundary THEN travel 1.5x base range
        """
        ...
        required_breakout_distance = base_range * self.min_legout_ratio  # 1.5x base range
        ...
        for leg_length in range(1, max_leg_length + 1):
            ...
            if (self.is_valid_leg_in(leg_data, direction) and 
                ...
                # CRITICAL: Calculate breakout distance beyond zone boundary
                if direction == 'bullish':
                    # Must break above zone high, then travel 1.5x base range
                    leg_high = leg_data['high'].max()
                    
                    # Check if leg broke above zone
                    if leg_high <= zone_high:
                        continue  # Didn't break out of zone
                    
                    # Calculate distance traveled BEYOND zone boundary
                    breakout_distance = leg_high - zone_high
                    
                else:  # bearish
                    # Must break below zone low, then travel 1.5x base range
                    leg_low = leg_data['low'].min()
                    
                    # Check if leg broke below zone
                    if leg_low >= zone_low:
                        continue  # Didn't break out of zone
                    
                    # Calculate distance traveled BEYOND zone boundary
                    breakout_distance = zone_low - leg_low
                
                # STRICT VALIDATION: Breakout distance must be 1.5x base range
                if breakout_distance >= required_breakout_distance:
                    return {
                        'start_idx': start_idx,
                        'end_idx': end_idx,
                        'direction': direction,
                        'range': breakout_distance,
                        'ratio_to_base': breakout_distance / base_range,
                        'strength': self.calculate_leg_strength(leg_data, direction),
                        'candle_count': leg_length,
                        'breakout_distance': breakout_distance,
                        'required_distance': required_breakout_distance,
                        'zone_break_level': zone_high if direction == 'bullish' else zone_low
                    }
        
        return None  # Failed to meet breakout distance requirement

    def detect_dbd_patterns_enhanced(self, data: pd.DataFrame) -> List[Dict]:
        """
        Enhanced D-B-D detection with base boundary validation
        """
        patterns = []
        ...
        for i in range(len(data) - 4):
            try:
                ...
                # Phase 3: Check for zone invalidation BEFORE leg-out
                pattern_preview = {
                    'type': 'D-B-D',
                    'base': base,
                    'zone_high': base['high'],
                ...
                # Phase 4: Validate bearish leg-out with BREAKOUT distance requirement
    ...
    def detect_rbr_patterns_enhanced(self, data: pd.DataFrame) -> List[Dict]:
        """
        Enhanced R-B-R detection with base boundary validation
        """
        patterns = []
        ...
        for i in range(len(data) - 4):
            try:
                ...
                # Phase 4: Validate bullish leg-out with BREAKOUT distance requirement
                leg_out_start = base['end_idx'] + 1
                leg_out = self.validate_leg_out_breakout_distance(
    ...
    def debug_pattern_validation(self, data: pd.DataFrame, start_idx: int, end_idx: int):
        """
        Enhanced debug with base boundary and breakout validation
        """
        print(f"\n🔍 ENHANCED DEBUG - CANDLES {start_idx}-{end_idx}")
        print("=" * 80)
        ...
        for i in range(start_idx, min(end_idx + 1, len(data))):
            ...
            print(f"Candle {i:2d}: {classification:9s} | {direction:7s} | Ratio: {ratio:.3f} | "
                ...
                f"Range: {total_range:.5f}")
        
        # Test specific patterns in this range
        print(f"\n🔍 TESTING PATTERNS IN RANGE {start_idx}-{end_idx}")
        
        for i in range(start_idx, min(end_idx - 3, len(data))):
            # Test leg-in
            leg_in_bull = self.identify_leg_in(data, i, direction='bullish')
            leg_in_bear = self.identify_leg_in(data, i, direction='bearish')
            
            if leg_in_bull:
                print(f"  🟢 Bullish leg-in at {i}: {leg_in_bull['start_idx']}-{leg_in_bull['end_idx']}")
                
                # Test base
                base_start = leg_in_bull['end_idx'] + 1
                if base_start < len(data):
                    base = self.identify_base(data, base_start)
                    if base:
                        print(f"     📦 Base found: {base['start_idx']}-{base['end_idx']}")
                        
                        # Test base boundaries
                        base_valid, base_msg = self.validate_base_boundaries(leg_in_bull, base, data)
                        print(f"     🔲 Base boundaries: {base_msg}")
                        
                        if base_valid:
                            # Test leg-out
                            leg_out_start = base['end_idx'] + 1
                            if leg_out_start < len(data):
                                leg_out = self.validate_leg_out_breakout_distance(
                                    data, leg_out_start, base, direction='bullish'
                                )
                                if leg_out:
                                    print(f"     🚀 Leg-out found: Breakout distance {leg_out['breakout_distance']:.5f}")
                                else:
                                    print(f"     ❌ Leg-out failed: Insufficient breakout distance")
            
            if leg_in_bear:
                print(f"  🔴 Bearish leg-in at {i}: {leg_in_bear['start_idx']}-{leg_in_bear['end_idx']}")
    ...
    def validate_leg_out_distance(self, data: pd.DataFrame, start_idx: int, 
                                 base_range: float, direction: str) -> Optional[Dict]:
        """
        STRICT leg-out validation - must travel 1.5x base range minimum
        """
        if start_idx >= len(data):
            return None
            
        max_leg_length = 5
        min_leg_out_range = base_range * self.min_legout_ratio  # STRICT 1.5x requirement
        ...
        for leg_length in range(1, max_leg_length + 1):
            ...
            if (self.is_valid_leg_in(leg_data, direction) and 
                ...
                # CRITICAL: Calculate actual distance traveled
                if direction == 'bullish':
                    leg_start = leg_data['low'].iloc[0]  # Start from low of first candle
                    leg_end = leg_data['high'].max()     # End at highest point
                    leg_range = leg_end - leg_start
                else:
                    leg_start = leg_data['high'].iloc[0] # Start from high of first candle
                    leg_end = leg_data['low'].min()      # End at lowest point
                    leg_range = leg_start - leg_end
                
                # STRICT VALIDATION: Must meet 1.5x minimum
                if leg_range >= min_leg_out_range:
                    return {
                        ...
                        'strength': self.calculate_leg_strength(leg_data, direction),
                        'candle_count': leg_length,
                        'actual_distance': leg_range,
                        'required_distance': min_leg_out_range
                    }
        
        return None  # Failed to meet distance requirement
