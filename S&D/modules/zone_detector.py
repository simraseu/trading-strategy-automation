...
class ZoneDetector:
   ...
   def detect_all_patterns(self, data: pd.DataFrame) -> Dict[str, List[Dict]]:
    ...
    try:
        ...
        # REMOVED: overlap prevention for testing purposes
        # You can add this back in backtesting module:
        # dbd_patterns = self.remove_overlapping_patterns(dbd_patterns)
        # rbr_patterns = self.remove_overlapping_patterns(rbr_patterns)
       ...
       for leg_length in range(1, max_leg_length + 1):
           ...
           if (self.is_valid_leg_in(leg_data, direction) and 
               ...
               if leg_range >= min_leg_out_range:
                   ...
                   distance_ratio = leg_range / base_range
                   distance_score = self.calculate_distance_score(distance_ratio)
                   ...
                   return {
                       ...
                       'direction': direction,
                       'range': leg_range,
                       'ratio_to_base': distance_ratio,
                       'strength': max(self.calculate_leg_strength(leg_data, direction), distance_score),
                       'candle_count': leg_length,
   ...
   def calculate_distance_score(self, distance_ratio: float) -> float:
       """
       Distance traveled priority scoring (your manual rule: distance matters most)
       """
       if distance_ratio >= 4.0:      # 4x+ base = exceptional
           return 1.0
       elif distance_ratio >= 3.0:    # 3x+ base = very strong  
           return 0.9
       elif distance_ratio >= 2.5:    # 2.5x+ base = strong
           return 0.8
       elif distance_ratio >= 2.0:    # 2x+ base = acceptable
           return 0.7
       else:                          # <2x base = weak
           return 0.4
   ...
   def calculate_base_quality(self, base_data: pd.DataFrame) -> float:
       """Calculate base quality with your manual trading sweet spots"""
       candle_count = len(base_data)
       
       # YOUR MANUAL SWEET SPOT RULES:
       if candle_count <= 2:          # Sweet spot - best probability
           count_score = 1.0
       elif candle_count <= 4:        # Good probability  
           count_score = 0.8
       elif candle_count <= 6:        # Acceptable probability
           count_score = 0.6
       else:                          # Really bad probability (7+)
           count_score = 0.2
       ...
       return (count_score * 0.7) + (range_score * 0.3)
   
   def calculate_pattern_strength(self, leg_in: Dict, base: Dict, leg_out: Dict) -> float:
       """Calculate pattern strength with leg-out priority (your manual trading rules)"""
       ...
       # Base bonus (your sweet spot preference)
       base_bonus = 1.0 if base['candle_count'] <= 3 else 0.7
       
       # Distance traveled bonus (leg-out ratio importance)
       distance_bonus = min(leg_out_ratio / 1.5, 1.0)
       
       # YOUR MANUAL PRIORITY WEIGHTING:
       # Leg-out: 50% (most important by far)
       # Base: 25% (important but not critical)  
       # Leg-in: 15% (lowest priority)
       # Distance: 10% (inefficiency bonus)
       strength = (
           leg_out_strength * 0.50 +     # Most important by far
           base_quality * 0.25 +         # Important but not critical
           leg_in_strength * 0.15 +      # Lowest priority
           distance_bonus * 0.10         # Distance traveled bonus
       ) * base_bonus
       
       return round(strength, 3)
   
   def remove_overlapping_patterns(self, patterns: List[Dict]) -> List[Dict]:
    """Remove overlapping patterns using Level-On-Top-Of-Level rule (furthest zone wins)"""
    if len(patterns) <= 1:
        return patterns
    
    # Sort by pattern start index (process chronologically)
    ...
    for pattern in sorted_patterns:
        ...
        for i, existing in enumerate(filtered):
            # Check if patterns overlap by more than 2 candles
            overlap_start = max(pattern['start_idx'], existing['start_idx'])
            overlap_end = min(pattern['end_idx'], existing['end_idx'])
            overlap_length = overlap_end - overlap_start + 1
            
            if overlap_length > 2:  # Patterns overlap
                # LEVEL-ON-TOP-OF-LEVEL RULE: Keep the furthest zone
                if pattern['type'] == 'D-B-D' and existing['type'] == 'D-B-D':
                    # For supply zones (D-B-D): keep the higher zone
                    if pattern['zone_high'] > existing['zone_high']:
                        filtered[i] = pattern  # Replace with higher zone
                    # else: keep existing (it's higher)
                    overlap = True
                    break
                    
                elif pattern['type'] == 'R-B-R' and existing['type'] == 'R-B-R':
                    # For demand zones (R-B-R): keep the lower zone  
                    if pattern['zone_low'] < existing['zone_low']:
                        filtered[i] = pattern  # Replace with lower zone
                    # else: keep existing (it's lower)
                    overlap = True
                    break
                    
                elif pattern['type'] != existing['type']:
                    # Different types: keep highest strength (existing logic)
